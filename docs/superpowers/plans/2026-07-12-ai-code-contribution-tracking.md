...
Before risky end capture/diff work, atomically persist deterministic fallback state plus a random durable claim token, monotonically increasing claim generation, and 5-second UTC lease. Same-operation racers return in-progress while the lease is live; after expiry, a retry automatically rotates the token, increments the generation, renews the lease, and executes the same fallback without developer action or sleeping. Exact/degraded replacement must compare-and-set the caller's token and generation so an expired original owner cannot overwrite a takeover and instead converges to the current persisted/completed plan. A capture failure, diff failure, or failure while writing the degradation plan still closes the claimed interval with the same `recovery_detected`/`UNKNOWN` result on retry. Later edits must never be absorbed into that failed transaction, and the lease adds no wait to the normal end path.
