...
- A transaction boundary alone is not evidence that a changed line came from the generation skill. Before `begin`, the skill must automatically add HMAC-only fingerprints for the exact proposed patch it will apply to the temporary evidence file. Only matching diff lines may continue through normal `USER_SUPPLIED`/`AI_REUSED`/`AI_SKILL` classification; unmatched or missing applied-patch evidence is `UNKNOWN`.
...
2. Generate the proposed patch in memory without changing the repository. Automatically build one temporary HMAC-only evidence file containing separate fingerprints/counts for complete prompt-supplied code and for the exact proposed patch lines. Never put raw prompt text or raw code in that file, and never ask the developer to create or confirm it.
...
5. End the transaction after application. Record the actual net patch, patch hash, before/after blobs, classifications, counts, and validation outcome. Prompt overlap remains first for evidenced lines; repository reuse and ordinary AI classification follow. Any diff line not matched by applied-patch evidence is `UNKNOWN`. Abort the transaction when no patch was applied.
6. Queue events locally and upload asynchronously. Recorder or server failure must not block coding; automatically close the affected interval as degraded `UNKNOWN` evidence rather than letting a later retry absorb more edits.
